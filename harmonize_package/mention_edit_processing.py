...
class MentionEditProcessor:

    def __init__(self, form, blog_entry_id, user_id):
    ...
    def set_blog_user_mentions_for_edit(self):
        if len(self.validated_mentions) != 0:
            ...
            num_del_rows = queries.remove_all_mentions_of_alias_for_blog(blog_entry_id=self.blog_entry_id)
            for alias in unique_validated_aliases:
                ...
                if queries.chk_if_blog_mention_row_exists(blog_entry_id=self.blog_entry_id, user_id=user_id):
                    continue
                else:
                    queries.set_user_blogs_mention(blog_entry_id=self.blog_entry_id, user_id=user_id)
        return
    ...
    def process_collected_mentions_for_edit(self):
        ...
        if len(self.user_mentions) != 0:
            ...
            for user_alias in self.user_mentions:
                ...
                if queries.get_user_alias(user_alias=user_alias):
                    ...
                    if self.form.entry_img.data:
                        ...
                        #entry = BlogEntry(title=self.form.title.data, body=corrected_entry_body, entry_img=picture_name, private_entry=self.form.private_entry.data, user_id=current_user.id)
                        # look up existing blog entry
                        blog_entry = BlogEntry.query.filter_by(id=self.blog_entry_id).first()
                        blog_entry.title = self.form.title.data
                        blog_entry.body = corrected_entry_body
                        blog_entry.entry_img = picture_name
                        blog_entry.private_entry = self.form.private_entry.data
                        ...
                        # update values in db
                        blog_entry.analysis_neg_score = polarity_scores['neg']
                        blog_entry.analysis_neu_score = polarity_scores['neu']
                        blog_entry.analysis_pos_score = polarity_scores['pos']
                        blog_entry.analysis_compound_score = polarity_scores['compound']
                        ...
                        return redirect(url_for('view_blog_entry', user_id=self.user_id, blog_entry_id=self.blog_entry_id))
                    else:
                        print('CASE: INVALID MENTION AND NO IMAGE')
                        # create a new entry with the suggestions for the incorrect user alias(s) not spelled right.
                        #entry = BlogEntry(title=self.form.title.data, body=corrected_entry_body, private_entry=self.form.private_entry.data, user_id=current_user.id)
            ...
            if self.form.entry_img.data:
                ...
                picture_name = img_processor.format_img_for_blog()
                ##################################################
                #blog_entry = BlogEntry(title=self.form.title.data, entry_img=picture_name, body=self.form.body.data, private_entry=self.form.private_entry.data, user_id=current_user.id)
                # add entry to session
                blog_entry = BlogEntry.query.filter_by(id=self.blog_entry_id).first()
                blog_entry.title = self.form.title.data
                blog_entry.body = self.form.body.data
                blog_entry.entry_img = picture_name
                blog_entry.private_entry = self.form.private_entry.data
                # update sentiment analysis values
                # instantiate analyzer
